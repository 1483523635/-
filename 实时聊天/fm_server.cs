...
namespace 实时聊天
{
    public partial class fm_server : Form
    {
        ...
        private void btn_recFile_Click(object sender, EventArgs e)
        {
            ...
            ThreadPool.QueueUserWorkItem(_ => RecFile(remoteClient_file, saveFilePath));
        ...
        {
            string filePath = chat.GetSendFilePath(new FileHelper());
            List<TcpClient> RecverList = new List<TcpClient>();
            ChooseRec recver = new ChooseRec(ClientList_file);
            recver.ShowDialog();
            RecverList= recver.GetAllRecvier();
            ThreadPool.QueueUserWorkItem(_=> { chat.SendFile_Much(RecverList, filePath); });
        }
        #endregion
        #endregion
    }
